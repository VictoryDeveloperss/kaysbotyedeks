...
message.guild.roles.create({ data: { name: '・youtube.com/c/xVictoryy' }, reason: 'ayn' }).then(s => s.setColor('#f70c0c'))
message.guild.roles.create({ data: { name: '・Botlar' }, reason: 'ayn' }).then(s => s.setColor('#e77e2e'))
message.guild.roles.create({ data: { name: 'Kays Bot Başarıyla Sunucucunuzu Kurdu' }, reason: 'ayn' }).then(s => s.setColor('#ffffff'))
