...
exports.run = async(client, message, args) => {
 ...
 var sebeb = args.slice(1).join(" ");
    ...
    if(message.author == kisi) return message.reply("Kendini Yasaklayamazsın <:ReddedilmiPng:813007408217063424>!")
    if (banxx.get(kisi.id)) return message.reply("Kişi Zaten Yasaklanmış <:ReddedilmiPng:813007408217063424>!")
 ...
 if (!sebeb) {
         try {
          ...
          message.reply("Bir Sorun Oldu Lütfen Botun Geliştiricisi veya Yapımcısıyla İletişime Geçiniz <:ReddedilmiPng:813007408217063424>!")
          console.log(error)
        }
 } else {
 try {
   kisi.send(`${kisi} **${guild}** adlı sunucudan banlandınız <:ban:750398536180563989>. \nNedeni: **${sebeb}**`)
   message.channel.send(`**${kisi} banlandı. \nNedeni: ${sebeb}**`)
   guild.members.ban(kisi, { reason: sebeb/*, days: gun*/});
 } catch (error) {
   message.reply("Bir Sorun Oldu Lütfen Botun Geliştiricisi veya Yapımcısıyla İletişime Geçiniz <:ReddedilmiPng:813007408217063424>!")
