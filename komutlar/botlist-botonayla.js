 ...
 let westrabotlistyetkilisi = db.fetch(`westrabotlistyetkilirol_${message.guild.id}`)
  ...
  message.delete()
   const westrabumbeyyyy = new Discord.MessageEmbed()
    ...
    .setFooter(`Kays Bot`)
    .setTimestamp()
    .setDescription(`<a:tik:813007245440319499> <@${sahip}> adlı kişinin <@${botisim}> adlı botu onaylandı. Onaylayan yetkili: ${message.author}`)
		client.channels.cache.get(log).send(westrabumbeyyyy);
		message.channel.send(`<a:tik:813007245440319499> Botu onayladınız.`).then(x => x.delete({timeout: 3000}))
};
